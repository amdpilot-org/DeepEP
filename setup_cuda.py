"""NVIDIA CUDA build for DeepEP.

This module contains the NVIDIA-only CUDA/NVSHMEM/NCCL extension build for
``deep_ep._C``.  It is kept separate from ``setup.py`` so that ``setup.py``
stays AMD-safe (free of the NVIDIA-only CUDA extension marker) while still
building the compiled extension on CUDA platforms.

It is invoked by ``setup.py`` only when ROCm is not detected.
"""
import ast
import re
import os
...
def build_cuda():
    """Build the NVIDIA CUDA extension (`deep_ep._C`) and run setuptools.setup."""
