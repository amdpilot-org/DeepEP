...
def _is_rocm_backend() -> bool:
    """Detect an AMD ROCm / gfx platform (e.g. MI355X / gfx950).

    DeepEP routes its public API to MORI via the pure-Python
    ``deep_ep._amd_backend`` module on ROCm, avoiding the NVIDIA-only
    ``deep_ep._C`` compiled extension.  Detection uses ``torch.version.hip``
    (set on ROCm PyTorch builds) and the device ``gcnArchName``.
    """
    if getattr(getattr(torch, 'version', None), 'hip', None) is not None:
        return True
    try:
        if torch.cuda.is_available():
            arch = getattr(torch.cuda.get_device_properties(0),
                           'gcnArchName', '') or ''
...
if is_rocm:
    # AMD ROCm path: route the public DeepEP API to MORI through the
    # pure-Python `deep_ep._amd_backend` module.  No compiled CUDA/NVSHMEM
    # extension is used, so `import deep_ep` succeeds on MI355X / gfx950
    # without building the NVIDIA-only `deep_ep._C` extension.
    # noinspection PyUnresolvedReferences
    from deep_ep._amd_backend import (
        Buffer, Config, EventOverlap, EventHandle,
        ElasticBuffer, EPHandle,
        intranode_all_to_all, internode_all_to_all,
    )
else:
    # NVIDIA CUDA path (original implementation).
    import filecmp
    import functools
    import glob
    import subprocess
